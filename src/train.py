...
def train(train_loader, val_loader, model, criterion, optimizer, lr_scheduler, epochs, device, writer, metrics,
    ...
    for epoch in range(epochs):
        ...
        # best_loss, best_loss_epoch = check_best_loss(epoch, best_loss, val_loss, model, optimizer, log_dir, args, return_epoch=True, best_loss_epoch=best_loss_epoch)
        # epoch, best_loss, best_epoch, val_loss, model, optimizer, log_dir, args
        best_loss, best_loss_epoch = check_best_loss(epoch, best_loss, best_loss_epoch, val_loss, model, optimizer, log_dir, args)
...
def train_step(data_loader, model, criterion, optimizer, epoch, epochs, writer, device, metrics, stats,  args):
    ...
    for batch_idx, (data, target) in tqdm(enumerate(data_loader)):
        ...
        masks={}
        masks['positive']={'audio':p['audio_mask'], 'video':p['video_mask']} # masks from collate_fn
